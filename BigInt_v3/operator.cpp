...
bool BigInt::operator >=(const BigInt& b) {
    ...
    if(this->sign > b.sign) return true;
    if(this->sign < b.sign) return false;
    
    int abs_comp = this->ab_comp(b);
    if(abs_comp==0) return true;
    if((abs_comp==1 && this->sign==1) || (abs_comp==-1 && b.sign==0)) return true;
    return false;
